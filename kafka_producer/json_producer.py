...
def process_arguments(version_description, *args):
    ...
    _arguments = parser.parse_args(args)
    return _arguments


if __name__ == '__main__':
    version_description_global = 'JSONSerializer Kafka example'
    bootstrap_servers_cli = "http://127.0.0.1:9192"
    schema_registry_cli = "http://127.0.0.1:9192"
    topic_cli = "test"

    cli_args = tuple(f"-b {bootstrap_servers_cli} -s {schema_registry_cli} -t {topic_cli}".split())

    arguments = process_arguments(version_description_global, *cli_args)
